...
> {
  constructor(props: AccountPlansProps) {
    ...
    const fa: AccountModificationFunctions = {
      activate: (id) => {
        ...
        .then((plans) => {
          ...
          getDocument('schedules')
            ...
            .catch((error: PaperError) => {
              this.props.alert(errorAlert('account_get_plans', error));
            });
        })
        .catch((error: PaperError) => {
          this.props.alert(errorAlert('account_get_plans', error));
  ...
  create(isSchedule: boolean) {
    ...
    this.props.alert({
      ...
      action: ({ inputText: name }) => {
        if (!name) {
          this.props.alert(
            errorAlert(`account_create_${t}`, new PaperError('No Name'))
          );
          return;
        }
        ...
        toast.promise(
          createDocument(isSchedule ? 'schedules' : 'plans', name),
          {
            ...
            error: (error: PaperError) => {
              this.props.alert(errorAlert(`account_create_${t}`, error));
  ...
  edit(id: string, name: string) {
    ...
    this.props.alert({
      ...
      action: ({ inputText: newName }) => {
        if (!newName) {
          this.props.alert(
            errorAlert(`account_edit_${t}`, new PaperError('No Name'))
          );
          return;
        }
        this.setState({ loading: true });
        toast.promise(
          ...
          }),
          {
            ...
            error: (error: PaperError) => {
              this.props.alert(errorAlert(`account_edit_${t}`, error));
              return 'Something went wrong';
            },
          }
        );
      ...
      action: ({ inputText: name }) => {
        ...
        toast.promise(
          ...
          }),
          {
            ...
            error: (error: PaperError) => {
              this.props.alert(errorAlert(`account_duplicate_${t}`, error));
              return 'Something went wrong';
            },
          }
        );
      ...
      action: () => {
        ...
        toast.promise(
          deleteDocument(isSchedule ? 'schedules' : 'plans', id),
          {
            ...
            error: (error: PaperError) => {
              this.props.alert(errorAlert(`account_delete_${t}`, error));
              return 'Something went wrong';
            },
          },
          {
          ...
          .map((schedule, i) => {
            return (
              <AccountPlan
                ...
                plan={schedule}
                fa={this.state.fa}
                active={schedule.id === this.props.activeId}
          ...
          .map((plan, i) => {
            return (
              <AccountPlan
                ...
                plan={plan}
                fa={this.state.fa}
                active={plan.id === this.props.activeId}
                key={`account-plan-${i}`}
              />
            );
          }) || [];
