...
for link_major in link_all_majors:
    ...
    for major_class in major_classes:
        ...
        for course_extra in course_extras:
            text = course_extra.get_text().strip().lower().replace('&nbsp;', ' ').replace('\xa0', ' ')

            if 'distro' not in text: continue
        ...
        course_data[index]['distros'] = distros
        count += 1

    print('Scraped distro data for ' + str(count) + ' ' + code + ' courses.')
